...
<div x-cloak class="mt-6" x-data="{ open: false }" 
    x-init="if (!sessionStorage.getItem('modal_shown_{{ $modal->id }}')) { 
        setTimeout(() => open = true, 50); 
        sessionStorage.setItem('modal_shown_{{ $modal->id }}', 'true'); 
    }">
    <div class="fixed z-50 top-0 left-0 flex items-center justify-center w-full h-full px-4"
        ...
        <div class="relative h-auto max-h-[90dvh] overflow-y-auto px-4 pt-6 pb-4 mx-2 text-left bg-white rounded-lg shadow-2xl md:w-4/12 lg:px-8 md:mx-0"
