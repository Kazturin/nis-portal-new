...
<div class="container mx-auto mb-24 mt-12">
    <div>
                    ...
                    <div class="w-full lg:w-5/12 px-4">
                        @foreach($sideNews as $val)
                            <div class="flex flex-col lg:flex-row space-x-2 rounded-3xl overflow-hidden lg:h-1/4">
                                <img class="flex-shrink-0 w-full lg:w-4/12 rounded-xl h-full object-cover px-1 py-2"
                                    src="{{ $val->getPhoto() }}" alt="">
        ...
        <x-animate-on-scroll animation="fade-in-up" class="text-center">
            <a class="font-inter text-xl block w-fit mx-auto py-2 px-4 my-4 bg-primary hover:bg-[#67A600] text-white rounded-3xl"
                href="{{ route('news', $locale) }}">{{ __("All news") }}</a>
        </x-animate-on-scroll>
    </div>
</div>