        ...
        <div class="p-5">
            <div class="flex justify-center items-start my-2">
                <div class="max-w-screen-lg">
                    <x-animate-on-scroll animation="fade-in-up" tag="h2" class="font-inter text-3xl xl:text-4xl mb-16 text-center">
                        {{ __("Frequently asked questions") }}
                    </x-animate-on-scroll>

                    <x-animate-on-scroll animation="scale-in" tag="ul" class="flex flex-col rounded-2xl overflow-hidden">
                        @forelse ($faq as $item)
                            <li class="my-1 bg-secondary px-5 py-8" x-data="accordion({{ $item->id }})">
                                <h2 @click="handleClick()" class="flex flex-row justify-between items-center font-semibold cursor-pointer">
                        ...
                        @empty
                            <p class="text-sf text-lg">{{ __('Empty') }} ...</p>
                        @endforelse
                    </x-animate-on-scroll>
                </div>
            </div>
